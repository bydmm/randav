 ...
 *       Filename:  randav.c
 *
 *    Description:  rand open a video file 
 *
 *        Version:  1.0
 *        Created:  2013.2.1
 *       Compiler:  gcc
...
void
videolist(char temp[256])
{
	 if(WildCharMatch(temp, "*.rmvb", 1)
	           || WildCharMatch(temp, "*.mp4", 1)
						 || WildCharMatch(temp, "*.avi", 1)
						 || WildCharMatch(temp, "*.wmv", 1))
	 {
		 sprintf(filelist[end], "%s", temp);
		 end++;
	 }	
}
